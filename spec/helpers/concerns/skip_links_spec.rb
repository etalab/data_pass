RSpec.describe SkipLinks do
        ...
        context 'with a non-whitelisted page' do
          before do
            allow(self).to receive_messages(
              whitelisted_for_default_skip_links?: false,
              renders_html_view?: true,
          ...
          it 'raises an error' do
            expect { skip_links_content }.to raise_error(RuntimeError, /No skip links defined for this page/)
          end
        end

        context 'with a whitelisted page' do
          before do
            allow(self).to receive(:whitelisted_for_default_skip_links?).and_return(true)
          end

          it 'returns the default skip links' do
            result = skip_links_content
            expect(result).to include('Menu')
            expect(result).to include('Pied de page')
          end
        end
  ...
  describe '#whitelisted_for_default_skip_links?' do
    it 'returns true for whitelisted pages' do
      ...
      expect(whitelisted_for_default_skip_links?).to be true
    end
    ...
    it 'returns false for non-whitelisted pages' do
      ...
      expect(whitelisted_for_default_skip_links?).to be false
    end
  end

  describe '#renders_html_view?' do
    ...
    context 'when request is HTML and not XHR' do
      ...
      it 'returns true' do
        expect(renders_html_view?).to be true
      end
    end
    ...
    context 'when request is XHR' do
      ...
      it 'returns false' do
        expect(renders_html_view?).to be false
      end
    end
