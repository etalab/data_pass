...
RSpec.describe ApplicationHelper do
  describe '#linkify_urls' do
    ...
    it 'linkifies multiple URLs' do
      ...
      result = helper.linkify_urls(text)
      expect(result).to include('href="https://a.com"')
      expect(result).to include('href="https://b.com"')
    end

    it 'strips trailing punctuation from the href but preserves it as text' do
      expect(helper.linkify_urls('Voir https://example.com.')).to include('href="https://example.com"')
      expect(helper.linkify_urls('Voir https://example.com.')).to include('</a>.')
      expect(helper.linkify_urls('Super, https://example.com/path,')).to include('href="https://example.com/path"')
      expect(helper.linkify_urls('Super, https://example.com/path,')).to include('</a>,')
    end
    ...
    it 'prevents attribute breakout when URL contains a quote' do
      ...
      expect(anchor['href']).to include('https://evil.com')
    end
