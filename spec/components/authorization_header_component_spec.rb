RSpec.describe AuthorizationHeaderComponent, type: :component do
  ...
  describe 'styling based on authorization state' do
    ...
    describe '#header_text_class' do
      ...
      context 'when authorization is revoked' do
        ...
        it 'returns mention grey text class with revoked modifier' do
          expect(component.header_text_class).to eq('fr-text-mention-grey authorization-header__text--inactive')
        end
      end
    ...
    describe '#button_class' do
      let(:base_classes) { %w[fr-btn fr-btn--sm fr-btn-secondary] }

      context 'when authorization is active' do
        before { authorization.state = 'active' }

        it 'returns unchanged classes' do
          expect(component.button_class(base_classes)).to eq(base_classes)
        end
      end
      ...
      context 'when authorization is revoked' do
        ...
        it 'removes fr-btn-secondary class' do
          expect(component.button_class(base_classes)).not_to include('fr-btn-secondary')
          expect(component.button_class(base_classes)).to eq(%w[fr-btn fr-btn--sm])
        end
      end
