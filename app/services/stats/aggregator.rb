...
module Stats
  class Aggregator
    ...
    def mode_time_to_first_instruction
      calculate_mode(authorizations_with_submit_and_first_instruction_events, time_difference_sql('first_instruction_events', 'submit_events'), 86_400)
    end
    ...
    def reopen_events_count
      ...
      reopen_events.count do |reopen_event|
    ...
    def active_authorizations_by_organization_type
      Authorization
        .validated
    ...
    def volume_by_type_with_states
      ...
      validated_refused = @authorization_requests
        .where(state: %w[validated refused])
        .group(:type, :state)
        .count
    ...
    def volume_by_provider_with_states
      ...
      validated_refused.each do |(type, state), count|
        ...
        provider_name = if definition&.provider
      ...
      # Convert to array and calculate totals
      by_provider.map { |provider, states|
        {
          provider: provider,
          validated: states[:validated],
          refused: states[:refused],
    ...
    def mode_time_to_production_instruction
      calculate_mode(authorizations_with_start_next_stage_and_production_instruction_events, time_difference_sql('first_production_instruction_events', 'start_next_stage_events'), 86_400)
    end
    ...
    def calculate_percentile(scope, time_expression)
      result = scope
        .pick(Arel.sql("PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY #{time_expression})"))
    ...
    def calculate_stddev(scope, time_expression)
      result = scope
        .pick(Arel.sql("STDDEV_POP(#{time_expression})"))
    ...
    def calculate_median_by_type(scope, time_expression)
      ...
      by_type.map { |type, times|
        {
          type: type,
          median_time: calculate_array_median(times),
          count: times.length
        }
      }.sort_by { |item| item[:median_time] }
    end
    ...
    def calculate_median_by_provider(scope, time_expression)
      ...
      by_provider.map { |provider, times|
        {
          provider: provider,
          median_time: calculate_array_median(times),
          count: times.length
    ...
    def production_authorization_requests
      ...
      production_types = AuthorizationDefinition.all.select { |def_item|
        def_item.stage.exists? && def_item.stage.type == 'production'
      }.map(&:authorization_request_class_as_string)
