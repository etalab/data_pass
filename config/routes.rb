Rails.application.routes.draw do
  ...
  scope(path_names: { new: 'nouveau', edit: 'modifier' }) do
    ...
    resources :authorizations, only: :show, path: 'habilitations' do
      resources :messages, only: %w[index create], path: 'messages'
      resources :authorization_request_events, only: :index, path: 'historique', as: :events
      resources :authorizations, only: :index, path: 'associees', as: :related_authorizations
      resources :france_connected_authorizations, only: :index, path: 'habilitations-france-connectees'
    end
  ...
  namespace :instruction do
    ...
    resources :authorization_definitions, only: %i[index show], path: 'formulaires' do
      resource :blocks, only: :show, path: 'etapes', controller: 'authorization_definition_blocks'
  ...
  namespace :admin do
    ...
    resources :bans, only: %i[index new create destroy], path: 'bannissements' do
      member do
        get :confirm_destroy
      end
    end
  ...
  namespace :api do
    ...
    namespace :v1 do
      ...
      resources :authorization_requests, path: 'demandes', only: %i[index show create update] do
