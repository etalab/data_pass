RSpec.describe SearchEngineBuilder do
  ...
  describe '#build_search_engine' do
    let!(:user_request) { create(:authorization_request, :api_entreprise, applicant: current_user, organization:) }
    let!(:other_request) { create(:authorization_request, :api_particulier, applicant: other_user) }
    let(:base_items) { AuthorizationRequest.all }

    context 'without search parameters' do
      let(:params) { ActionController::Parameters.new({}) }

      it 'returns all items with default sorting' do
        result = service.build_search_engine(base_items)

        expect(result).to contain_exactly(user_request, other_request)
    ...
    context 'with user_relationship_eq filter' do
      context 'when filtering by applicant' do
        ...
        it 'returns only items where current user is applicant' do
          result = service.build_search_engine(base_items)
          expect(result).to contain_exactly(user_request)
        end
      end
      ...
      context 'when filtering by organization' do
        ...
        it 'returns only items where current user is not applicant' do
          result = service.build_search_engine(base_items)
          expect(result).to contain_exactly(other_request)
        end
      end

      context 'when filtering by contact with AuthorizationRequest' do
        let(:params) do
          ActionController::Parameters.new(
        ...
        it 'applies mention query filtering' do
    ...
    context 'with text search' do
      let(:params) do
        ActionController::Parameters.new(
          search_query: { within_data_or_api_service_name_or_id_cont: 'search_term' }
        )
      end

      it 'applies text search filtering' do
        allow(base_items).to receive(:search_by_query).with('search_term')
          .and_return(base_items.where(id: user_request.id))
    ...
    context 'with ransack parameters' do
      ...
      it 'applies ransack filtering excluding specific parameters' do
    ...
    context 'with Authorization items and contact filter' do
      ...
      it 'returns only authorizations where current user is mentioned as contact' do
        result = service.build_search_engine(base_items)
