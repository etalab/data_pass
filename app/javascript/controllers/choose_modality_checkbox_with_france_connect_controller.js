...
export default class extends Controller {
  ...
  connect () {
    this.updateFranceConnectVisibility()
  }
  ...
  updateFranceConnectVisibility () {
    const checkedModalities = this.modalityTargets.filter(modality => modality.checked)
    const modalityValues = checkedModalities.map(modality => modality.value)
    const hasFranceConnect = modalityValues.includes('france_connect')
    ...
    this.updateLinksWithModalities(modalityValues)
  }

  updateLinksWithModalities (modalityValues) {
    this.linksTargets.forEach((link) => {
      const url = new URL(link.href)
      url.searchParams.delete('attributes[modalities][]')

      modalityValues.forEach(value => {
        url.searchParams.append('attributes[modalities][]', value)
      })
