...
RSpec.describe Developer::ReplayWebhookCall, type: :interactor do
  describe '.call' do
    subject(:result) { described_class.call(webhook_call_id: webhook_call.id) }
    ...
    context 'when webhook call does not exist' do
      subject(:result) { described_class.call(webhook_call_id: 999_999) }

      it 'fails with error' do
        expect(result).to be_failure
        expect(result.error).to eq(:webhook_call_not_found)
      end
    end
