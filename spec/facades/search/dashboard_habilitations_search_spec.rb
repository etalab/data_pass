...
RSpec.describe Search::DashboardHabilitationsSearch do
  ...
  describe 'includes associations' do
    it 'includes request and applicant' do
      search = described_class.new(user: user, params: params, scope: scope)
      
      # Verify that the search includes the expected associations
      expect(search.results.includes_values).to include(:request)
      expect(search.results.includes_values).to include(:applicant)
    end
  end

  describe 'basic filtering (with simple test data)' do
    let(:authorization_request) { create(:authorization_request, applicant: user, organization: organization, fill_all_attributes: false) }
    let!(:user_authorization) { create(:authorization, request: authorization_request, applicant: user) }

    it 'finds authorizations for the user' do
      search = described_class.new(user: user, params: params, scope: scope)
