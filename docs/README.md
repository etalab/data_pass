# Documentations

## Conception

* [Conception technique/métier](./conception.md)
* [Webhooks](./webhooks.md)

## Technique

* [Ajout d'un nouveau Data Provider (fournisseur de données)](./technique/ajout_nouveau_provider.md)
* [Ajout d'une nouvelle habilitation complète](./technique/nouveau_type_d_habilitation.md)
* [Système HabilitationType dynamique — architecture](./technique/habilitation_type_dynamique.md)
* [Ajouter un block à une habilitation dynamique](./technique/ajout_block_dynamique.md)
* [Ajout d'un nouveau bloc de formulaire (YAML legacy)](./technique/ajout_nouveau_bloc_de_formulaire.md)
* [Ajout d'un nouvel événement](./technique/ajout_evenement.md)
* [Authentification des agents via ProConnect](./technique/authentification_proconnect.md)
* [Design System (installation, explications techniques)](./technique/design.md)
* [Modal dynamique](./technique/modal.md)
* [Guide sur les scopes](./technique/modification_scopes.md)
* [Configuration des scopes de formulaire (scopes_config)](./technique/scopes_config.md)
* [Templates de messages pour l'instruction](./technique/templates_messages.md)
* [Synchronisation DataGouv — habilitations](./technique/datagouv-habilitations-sync.md)
* [Utiliser notre API](./technique/api.md)
* [Pré-remplissage d'une demande via query params](./technique/pre_remplissage_query_params.md)
* [Migration de l'ancienne stack (v1 -> v2)](./../app/migration/)

## Métier / Utilisateurs

* [Configurer un type d'habilitation dynamique](./metier/configuration_type_habilitation_dynamique.md)
* [Ajout d'un nouveau formulaire (non-technique)](./metier/ajout_nouveau_form_non_tech.md)
* [Cycle de vie d'une demande](./metier/lifecycle_documentation.md)
* [Cycle de vie d'une demande avec stages](./metier/lifecycle_documentation_with_stages.md)
* [Comportement du dashboard utilisateur](./metier/dashboard-comportement-utilisateur.md)
* [Bannière d'annonce](./metier/banniere_annonce.md)
* [Modifications admin](./metier/admin_change.md)

## API Particulier

* [Éditeurs certifiés FranceConnect](./api_particulier/france_connect/editeurs_certifies.md)

## Développeurs externes (API consumers)

* [Tutoriel : exploiter l'API en lecture](./developers/tutorials/lecture.md)
* [Tutoriel : exploiter l'API en écriture](./developers/tutorials/ecriture.md)
* [Tutoriel : suivre les demandes via webhooks](./developers/tutorials/webhooks.md)
