...
RSpec.describe AuthorizationRequestStatsController do
  describe 'GET #processing_time' do
    ...
    let(:cache_key) { "processing_days_v2_#{authorization_request_class}" }
    let(:query_instance) { instance_double(Stats::TimeToFinalInstructionQuery) }

    before do
      Rails.cache.clear

      allow(Stats::TimeToFinalInstructionQuery).to receive(:new).with(
    ...
    context 'when the query succeeds' do
      before do
        allow(query_instance).to receive(:percentiles).and_return({ p50: processing_days * 1.day.to_f, p80: processing_days * 1.day.to_f * 1.5 })
    ...
    context 'when caching' do
      it 'caches the processing time for a week' do
        ...
        allow(query_instance).to receive(:percentiles).and_return({ p50: processing_days * 1.day.to_f, p80: nil })
      ...
      it 'returns cached result on subsequent calls' do
        ...
        expect(Stats::TimeToFinalInstructionQuery).not_to receive(:new)
        get :processing_time, params: { definition: 'api_entreprise' }
    ...
    context 'when there is no data' do
      before do
        allow(query_instance).to receive(:percentiles).and_return({ p50: nil, p80: nil })

        get :processing_time, params: { definition: 'api_entreprise' }
      end

      it 'renders the default processing time' do
        expect(response.body).to include("<turbo-frame id='processing-time'>~#{AuthorizationRequestStatsController::DEFAULT_PROCESSING_DAYS} jours</turbo-frame>")
      end
    end

    context 'when an error occurs' do
      before do
        allow(query_instance).to receive(:percentiles).and_raise(StandardError)
      end
