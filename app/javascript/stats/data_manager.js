...
export function updateTimeSeriesChart (canvasElement, timeSeries, existingChartInstance, chartLabels) {
  return renderTimeSeriesChart(canvasElement, timeSeries, existingChartInstance, chartLabels)
}
...
function updateSingleDurationCard (percentile50Target, percentile80Target, data) {
  ...
  if (p50Label) p50Label.classList.toggle('fr-hidden', !p50Seconds)

  const p80Paragraph = percentile80Target.closest('p')
  if (p80Paragraph) p80Paragraph.classList.toggle('fr-hidden', !p80Seconds)
}
