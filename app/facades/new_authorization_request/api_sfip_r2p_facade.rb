class NewAuthorizationRequest
  class APISFiPR2PFacade < Base
    ...
    def public_available_editor_forms
      AuthorizationRequestFormDecorator.decorate_collection(
        authorization_definition.public_available_forms.select { |form| form.uid.ends_with?('editeur') }
      )
    end
    ...
    def authorization_definition_sandbox
      AuthorizationDefinition.find('api_sfip_r2p_sandbox')
    end
  end
end
