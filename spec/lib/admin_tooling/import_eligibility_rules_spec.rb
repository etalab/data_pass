...
RSpec.describe AdminTooling::ImportEligibilityRules do
  let(:csv_content) do
    <<~CSV
      Nom API,ID Datapass,ID API,Public Cible,Eligible,phrase,CTA,Notes
      ...
      API Particulier,api-particulier,107959582842083703515711757520394806672,collectivité ou administration,Oui,"Que vous soyez une collectivité, une administration centrale... vous êtes éligible",Demander une habilitation API Particulier,
      ...
      API Entreprise,api-entreprise,227114281190751739760356458553733252313,collectivité ou administration,Oui,"En tant qu'administration, vous êtes éligible",Demander l'accès aux données,
      API Entreprise,api-entreprise,227114281190751739760356458553733252313,entreprise ou association,Peut-être,"Sous certaines conditions, vous pourriez être éligible",En savoir plus,
    CSV
  end
  ...
  describe '.call' do
    ...
    it 'correctly maps CSV data to YAML structure' do
      ...
      expect(first_rule['definition_id']).to eq('api-particulier')
      expect(first_rule['options']).to be_an(Array)
      expect(first_rule['options'].size).to eq(3)
    end

    it 'uses symbols for type attribute' do
      described_class.call(csv_url, output_path: output_file.path)
      yaml_data = YAML.load_file(output_file.path)
    ...
    it 'uses symbols for eligible attribute' do
      described_class.call(csv_url, output_path: output_file.path)
      yaml_data = YAML.load_file(output_file.path)
    ...
    it 'includes cta with content and empty url when CTA column has value' do
      described_class.call(csv_url, output_path: output_file.path)
      yaml_data = YAML.load_file(output_file.path)

      collectivite_option = yaml_data.first['options'].find { |opt| opt['type'] == :collectivite_ou_administration }
      expect(collectivite_option['cta']).to eq({ 'content' => 'Demander une habilitation API Particulier', 'url' => '' })
    end
    ...
    context 'when API has incomplete target audiences' do
      let(:csv_content) do
        <<~CSV
          ...
          API Complete,api-complete,123,collectivité ou administration,Oui,"Test",CTA text,
          API Complete,api-complete,123,entreprise ou association,Non,"Test",,
          API Incomplete,api-incomplete,456,particulier,Non,"Test",,
          API Incomplete,api-incomplete,456,collectivité ou administration,Oui,"Test",CTA text,
        CSV
      end
      ...
      it 'skips APIs without all 3 target audiences' do
        ...
        expect(yaml_data.first['definition_id']).to eq('api-complete')
      end
    end
    ...
    context 'when rows have empty ID Datapass' do
      let(:csv_content) do
        <<~CSV
          ...
          API Valid,api-valid,123,collectivité ou administration,Oui,"Test",CTA,
